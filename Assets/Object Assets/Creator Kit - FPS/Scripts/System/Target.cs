...
public class Target : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider projectile)
    {
        ...
        {
            //numberOfSTocksToSteal = Random.Range(1, GameObject.FindGameObjectsWithTag("Enemy").Length);
            numberOfSTocksToSteal = 1;
            currNumStocksToDelete = numberOfSTocksToSteal;
            timeToWaitForMessage = -1.0f;
            for (int i = 0; i < memeData.memeShares.Length; i++)
            {
                while (memeData.memeShares[i] > 0 && currNumStocksToDelete > 0)
                {
                    memeData.memeShares[i] -= 1;
                    currNumStocksToDelete -= 1;
                    stocksStolenFromSpecificMeme += 1;
                }
                if (stocksStolenFromSpecificMeme > 0)
                {
                    timeToWaitForMessage += 1.0f;
                    StartCoroutine(TheftMessage(i, stocksStolenFromSpecificMeme, timeToWaitForMessage));
                    stocksStolenFromSpecificMeme = 0;
                }
            }
        }
